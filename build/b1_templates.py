...
def vol_pz():
    ...
    b += [h2('1.1 Основание для проектирования'),
          ...
          h2('1.4 Состав проектной документации'),
          mark('{{ТАБЛИЦА:СОСТАВ_ПД}}'),
          h2('1.5 Сведения об инженерных изысканиях'),
          mark('{{БЛОК:ИЗЫСКАНИЯ}}'),
...
def vol_zam():
    return [
        h1('1 Сведения о рассмотренной документации'),
        mark('{{ТАБЛИЦА:ЗАМ_ИСТОЧНИК}}'),
        h1('2 Замечания'),
        tx('Замечания сформированы по результатам проверки проектной документации на соответствие Техническим требованиям ПАО «Россети» (ТТ № 282р), ПУЭ-7, Положению о составе разделов проектной документации (ПП РФ № 87), ГОСТ Р 21.101-2020 и отчёту владельца инфраструктуры по пункту 13 Правил.'),
        mark('{{ТАБЛИЦА:ЗАМЕЧАНИЯ}}'),
        h1('3 Заключение'),
        mark('{{БЛОК:ЗАМ_ЗАКЛЮЧЕНИЕ}}'),
        tx('Проверил ________________ {{ПРОВ_ФИО}}'),
    ]
...
VOLUMES = [
    ...
    ('tpl_zam.docx', 'ЗАМ', '',   'Лист замечаний по проектной документации', 'zam', vol_zam),
...
def build(fname, code, num, vtitle, pd, fn):
    stage = {'vk': 'ВХОДНОЙ КОНТРОЛЬ ИСХОДНЫХ ДАННЫХ', 'zam': 'РАССМОТРЕНИЕ ПРОЕКТНОЙ ДОКУМЕНТАЦИИ'}.get(pd) if isinstance(pd, str) else None
