...
function intake(ex, phys) {
  ...
  ex.wires.forEach(function (w) {
    ...
    if (/^А[СC]?\s/.test(w.mark) && (w.kv === null || w.kv <= 1) && kvList.indexOf(0.4) >= 0)
      add('wires', 'warn', tag + ' отнесён к ВЛ 0,4 кВ. Для ВЛ 0,4 кВ с СИП норма расстояния ОКСН — провод 0,4 м (ТТ № 282р, п. 3.2.2); для неизолированных проводов 0,4 кВ ТТ норму не устанавливают. Тип провода ВЛ 0,4 кВ подтвердить.', { ask: 'владельцу' });
  });
